          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {categories.map((category) => (
              <motion.div key={category.key} variants={itemVariants}>
                <div className="p-6 rounded-xl bg-gray-800 hover:shadow-lg transition-shadow">
                  <h3 className="text-xl font-bold text-purple-500 mb-4">
                    {category.label}
                  </h3>

                  <div className="flex flex-wrap gap-3">
                    {(Array.isArray(t(`skills.${category.key}Skills`))
                      ? t(`skills.${category.key}Skills`)
                      : []
                    ).map((skill: string) => (
                      <motion.span
                        key={skill}
                        whileHover={{ scale: 1.05, y: -5 }}
                        className="px-4 py-2 rounded-full text-sm font-medium transition-all bg-purple-900/40 text-purple-300 hover:bg-purple-800/60"
                      >
                        {skill}
                      </motion.span>
                    ))}
